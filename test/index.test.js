...
describe("arrayUnique", function () {
    ...
    M.create({}, function (error, doc) {
      ...
      doc.save(function (error) {
        ...
        doc.save(function (error) {
          assert.ok(error);
          assert.ok(
            error.errors["docArr"].message.indexOf("Duplicate values") !== -1,
        ...
        M.findById(doc, function (error, doc2) {
          ...
          doc1.save(function (error) {
            ...
            doc2.save(function (error) {
              assert.ok(error);
              assert.ok(
                error.message.indexOf("No matching document") !== -1,
                error.message
              );
              done();
            });
  ...
  it("nested", function (done) {
    ...
    m.save(function (error) {
      assert.ok(error);
      assert.ok(
        error.errors["nested.arr"].message.indexOf("Duplicate values") !== -1,
        error.errors["nested.arr"].message
      );
      assert.ok(
        error.errors["nested.docArr"].message.indexOf("Duplicate values") !==
          -1,
  ...
  it("with array set to null (gh-1)", function () {
    ...
    return M.create({ arr: ["foo"] }).then((res) => {
      res.arr = null;
      // Should succeed
      return res.save();
    });
